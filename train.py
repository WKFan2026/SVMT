...
if (train_parameters["enable_photo_loss_early_stopping"]
        and train_parameters["photo_loss_early_stopping_patience"] <= 0):
    raise ValueError("photo_loss_early_stopping_patience must be positive.")
if (train_parameters["enable_interval_checkpoint_saving"]
        and train_parameters["checkpoint_save_interval_epochs"] <= 0):
    raise ValueError("checkpoint_save_interval_epochs must be positive.")
...
for epoch in range(epoch, train_parameters["epochs"] + 1):
    ...
    for batch, (imgs, intrinsic, boundary, sv_masks) in enumerate(dataloader):
        ...
        tq.set_postfix(loss="avg: {:.5f} cur: {:.5f}".format(mean_loss, loss.item()),
                       ...
                       loss_photo='avg: {:.5f} cur: {:.5f}'.format(
                           mean_photo_loss,
                           photo_loss.item()),
                       learning_rate='{:.2e}'.format(current_learning_rate),
                       )
    ...
    if mean_photo_loss < best_photo_loss:
        ...
        should_stop_early = (
            train_parameters["enable_photo_loss_early_stopping"]
            and photo_loss_epochs_without_improvement >= train_parameters["photo_loss_early_stopping_patience"]
        )
    ...
    if (train_parameters["enable_interval_checkpoint_saving"]
            and (epoch + 1) % train_parameters["checkpoint_save_interval_epochs"] == 0):
        interval_model_path = log_root / 'checkpoint_epoch_{:04d}_model.pt'.format(epoch + 1)
        save_model(model=depth_pose_estimation_model, optimizer=optimizer,
                   epoch=epoch, step=step, model_path=interval_model_path,
                   validation_loss=mean_loss,
                   best_photo_loss=best_photo_loss,
                   photo_loss_epochs_without_improvement=photo_loss_epochs_without_improvement)
